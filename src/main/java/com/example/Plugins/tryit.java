...
public class tryit extends Bot implements Master {
    ...
    public void handleRequests(Update update, String cmd) {
        ...
        if(cmd.equalsIgnoreCase(getHandler()+"info "+username)){
            ...
            try {
                ...
                info += "Account type : " + client.actions().users().findByUsername(username).get().getUser().getAccount_type()+"\n";
                info += "Followers : " + client.actions().users().findByUsername(username).get().getUser().getFollower_count()+"\n";
                info += "Following : " + client.actions().users().findByUsername(username).get().getUser().getFollowing_count()+"\n";
                info += "Extrenal Link : " + client.actions().users().findByUsername(username).get().getUser().getExternal_url()+"\n";
