...
public class post extends testBot implements Master {
    @Override
    public void handleRequests(Update update, String cmd) {
        ...
        if (update.getMessage().getReplyToMessage().hasVideo() && cmd.equalsIgnoreCase(getHandler() + "post")) {
            String username = "hellion_coder";
            String password = "aryan2702";
            IGClient client;
            try {
                try {
                    ...
                    long start = System.currentTimeMillis();
                    Video v = update.getMessage().getReplyToMessage().getVideo();

                    String caption = update.getMessage().getReplyToMessage().getCaption();

                    GetFile getFiled = new GetFile();
                    getFiled.setFileId(v.getFileId());
                    ...
                    editMessageText.setChatId(chatId(update));
                    editMessageText.setMessageId(m.getMessageId());
                    editMessageText.setText("Downloaded in " + elapsedTime + " millisecond");
                    ...
                    client.actions().timeline()
                            .uploadVideo(file2, file2, caption)
                            .thenAccept(response -> {
                                EditMessageText editMessageText2 = new EditMessageText();
                                editMessageText2.setChatId(chatId(update));
                                editMessageText2.setMessageId(m2.getMessageId());
